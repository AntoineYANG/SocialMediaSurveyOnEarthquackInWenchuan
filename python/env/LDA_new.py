...
jieba.suggest_freq('海地~地震', True)
jieba.suggest_freq('青海祈福', True)
jieba.suggest_freq('汶川地震', True)
jieba.suggest_freq('512汶川地震', True)
jieba.suggest_freq('地震', True)
jieba.suggest_freq('世博', True)
jieba.suggest_freq('母亲节', True)
jieba.suggest_freq('考研', True)
jieba.suggest_freq('地震女孩', True)
jieba.suggest_freq('汶川你好吗', True)
jieba.suggest_freq('我看90后', True)
jieba.suggest_freq('温州动车追尾', True)
jieba.suggest_freq('李宇春', True)
jieba.suggest_freq('韩庚', True)
jieba.suggest_freq('512地震发生时你在做什么', True)
jieba.suggest_freq('祭汶川停更512秒', True)
jieba.suggest_freq('苏宁易购', True)
jieba.suggest_freq('四川雅安地震', True)
jieba.suggest_freq('加多宝', True)
jieba.suggest_freq('我有话说', True)
jieba.suggest_freq('我在看新闻', True)
jieba.suggest_freq('鹿晗', True)
jieba.suggest_freq('汶川地震物资霉烂', True)
jieba.suggest_freq('黄子韬', True)
jieba.suggest_freq('致你爱的独白', True)
jieba.suggest_freq('宋茜', True)
jieba.suggest_freq('微公益', True)
jieba.suggest_freq('←这是在辟谣', True)
jieba.suggest_freq('汶川记忆', True)
jieba.suggest_freq('历史上的今天', True)
jieba.suggest_freq('那年今日', True)
jieba.suggest_freq('好人365', True)
jieba.suggest_freq('黄宇航', True)
jieba.suggest_freq('九寨沟地震', True)
jieba.suggest_freq('汶川地震时你在做什么', True)
jieba.suggest_freq('王源', True)
jieba.suggest_freq('映秀好人杨云青病逝', True)
...
if __name__ == '__main__':
    tree = [[[] for city in cityset] for month in range(0, 132)]
    ...
    for year in range(2009, 2020):
        with open('../../data/read/read{}.dat'.format(year), mode='r', encoding='utf-8') as file:
            ...
            for line in lines:
                ...
                city = box[2]
                try:
                    # print(tree[int(month)][int(posof[city])])
                    # print(content)
                    # print(month, city, posof[city], content)
                    # tree[int(month)][int(posof[city])] = ";".join([tree[int(month)][int(posof[city])], content])
                    tree[int(month)][int(posof[city])].append(content)
                    # print(">>>" + tree[int(month)][int(posof[city])])
                except IndexError:
                    print(month, city, posof[city])
                except KeyError:
                    print("KeyError: " + city)
                pass
            pass

    for month in range(0, 132):
        # print(month)
        for region in posof:
            # print("{}:{}".format(region, len(tree[int(month)][int(posof[city])])), end=",")
            result = ",".join([ss for ss in jieba.cut(";".join(tree[int(month)][int(posof[city])])) if not ss in stopwords])
            result = result.encode('utf-8')
            bag.append(result)
            # print("{}:{}".format(region, result.decode('utf-8')), end=",")
            pass
        # print()
        pass
    ...
    for e, values in enumerate(lda.inference(corpus)[0]):
        print(int(unit / 36), posof[(unit % 36)])
        unit += 1
